...
function isAppParent(obj) {
  return (
    Object.prototype.toString.call(obj) === "[Application Adobe Illustrator]"
  );
}
...
function objectToString(obj) {
  ...
  if (isArray(obj)) {
    ...
    if (isDefine(obj)) {
      // added--> to prevent null Error
      if (!isAdobeObject(obj)) {
        //if ENUM,  for example:  PointType.SMOOTH
  ...
  try {
    if (isDefine(obj)) {
      ...
      for (var key in obj) {
        if (!isParent(key)) {
          try {
            if (isObject(obj[key]) && isDefine(obj[key])) {
              // added ' && key!=="parent' ----> in order to Prevent Infinite loop
